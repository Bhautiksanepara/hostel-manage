...
function closeThemedControls(exceptElement = null) {
    document.querySelectorAll('.themed-select.open, .themed-date.open, .themed-time.open').forEach((wrapper) => {
...
document.addEventListener('click', (event) => {
    if (!event.target.closest('.themed-select') && !event.target.closest('.themed-date') && !event.target.closest('.themed-time')) {
        closeThemedControls();
    }
});
...
function initializeThemedSelects() {
    const selects = document.querySelectorAll('select:not(.themed-native-select)');
    ...
    selects.forEach((select) => {
        ...
        const syncTrigger = () => {
            ...
            const label = selectedOption ? selectedOption.textContent : 'Select an option';
        ...
        trigger.addEventListener('keydown', (event) => {
            if (event.key === 'ArrowDown' || event.key === 'Enter' || event.key === ' ') {
                ...
                const selectedButton = menu.querySelector('.is-selected') || menu.querySelector('.themed-select-option');
                if (selectedButton) {
        ...
        menu.addEventListener('keydown', (event) => {
            const buttons = Array.from(menu.querySelectorAll('.themed-select-option:not(:disabled)'));
            const currentIndex = buttons.indexOf(document.activeElement);

            if (event.key === 'ArrowDown') {
                event.preventDefault();
                const nextButton = buttons[Math.min(currentIndex + 1, buttons.length - 1)] || buttons[0];
                nextButton.focus();
            }

            if (event.key === 'ArrowUp') {
                event.preventDefault();
                const prevButton = buttons[Math.max(currentIndex - 1, 0)] || buttons[buttons.length - 1];
                prevButton.focus();
            }
...
function initializeThemedDatePickers() {
    const dateInputs = document.querySelectorAll('input[type="date"]:not(.themed-native-date)');
    ...
    dateInputs.forEach((input) => {
        ...
        const syncTrigger = () => {
            ...
            trigger.textContent = parsed ? formatDisplayDate(parsed) : 'Select a date';
...
function initializeThemedTimePickers() {
    const timeInputs = document.querySelectorAll('input[type="time"]:not(.themed-native-time)');
    ...
    timeInputs.forEach((input) => {
        ...
        const syncTrigger = () => {
            trigger.textContent = input.value ? formatDisplayTime(input.value) : 'Select a time';
